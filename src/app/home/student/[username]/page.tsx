...
import { useParams, useRouter } from "next/navigation"
import { useAuth } from "@/context/AuthContext"
import type { StudentProfile, UserProfile } from "@/types/user"
import AuthGuard from "@/components/auth/AuthGuard"
import { EditStudentProfile } from "@/components/profile/EditStudentProfile"
import { Button } from "@/components/ui/button"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
...
import { Badge } from "@/components/ui/badge"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Github, Edit, School, Plus, Share2, Calendar, ClipboardCheck } from "lucide-react"
import { auth, db } from "@/lib/firebase"
  ...
  return (
    <AuthGuard requireAuth={true} requireRole="student">
      <div className="max-w-md mx-auto">
        ...
        <div className="relative">
          <div className="h-40 bg-gradient-to-r from-neutral-900 to-neutral-800 w-full">
            <div className="absolute right-4 bottom-4 text-4xl md:text-4xl text-white">Student.io</div>
            {/* Edit and Share buttons */}
            <div className="absolute top-4 right-4 flex space-x-2">
              <Button
                size="icon"
        ...
        <div className="mt-12 px-4">
          ...
          <div className="mt-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Link href={`/home/student/${username}/attendance`}>
                <Button
                  variant="outline"
                  className="w-full flex items-center justify-center gap-2 h-auto py-3 border-blue-100 bg-blue-50 hover:bg-blue-100 text-blue-700"
                >
                  <ClipboardCheck className="h-5 w-5" />
                  <div className="flex flex-col items-start">
                    <span className="text-sm font-medium">Attendance</span>
                    <span className="text-xs text-blue-600">View your records</span>
                  </div>
                </Button>
              </Link>
              <Link href={`/home/student/${username}/timetable`}>
                <Button
                  variant="outline"
                  className="w-full flex items-center justify-center gap-2 h-auto py-3 border-purple-100 bg-purple-50 hover:bg-purple-100 text-purple-700"
                >
                  <Calendar className="h-5 w-5" />
                  <div className="flex flex-col items-start">
                    <span className="text-sm font-medium">Timetable</span>
                    <span className="text-xs text-purple-600">Class schedule</span>
                  </div>
                </Button>
              </Link>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {studentProfile?.skills?.map((skill, i) => (
                <Badge
                  key={i}
                  variant="outline"
                  className="bg-neutral-100 hover:bg-neutral-200 text-neutral-800 rounded-md font-normal border-neutral-200"
                >
                  {skill}
                </Badge>
              ))}
              {(!studentProfile?.skills || studentProfile.skills.length === 0) && (
                <div className="flex items-center justify-center w-full py-4 border border-dashed border-neutral-300 rounded-md">
                  <Button variant="ghost" size="sm" onClick={() => setIsEditing(true)} className="text-neutral-500">
                    <Plus className="h-4 w-4 mr-2" />
                    Add Skills
                  </Button>
                </div>
